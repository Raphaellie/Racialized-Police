...
pv.df <- read_csv("input/fatal shootings/Mapping Police Violence.csv") %>%
  select(name, age, gender, race, date, city,
    state2 = state, zip, county,
...
local.acs <-
  get_acs(
    geography = "place", year = 2020, output = "tidy", survey = "acs5",
...
local.race <-
  local.acs %>%
  mutate(
    state = str_extract(NAME, ", .*") %>% str_sub(start = 3),
    city = str_extract(NAME, "(.*),") %>% str_sub(end = -2)
  ) %>%
  mutate(
    state2 = state.abb[match(state, state.name)],
...
local.race <- local.race %>%
  rename(var = variable) %>%
  mutate(
    var = ifelse(var == "B01003_001", "all", var),
...
# Racialized Police ----
rp.df <- read_csv("input/LEMAS/LEMAS2020-cleaned.csv")

rp.df <- rp.df %>%
  rename_all(., .funs = tolower) %>%
  select(
    agency = agencyname, city, zipcode = zip, state2 = state, popserved = primarypop2020,
    sworn = ftsworn, sworn2 = totftemp,
    type = agencysamptype,
    contains(c("black_", "white_", "hisp_", "asian_"))
  ) %>%
  mutate(
    black_sworn = pers_black_male + pers_black_fem,
...
rp.cities <- reverse_zipcode(rp.df$zipcode) %>%
  select(zipcode,
    city2 = major_city, pop = population,
    lat, lon = lng, county = county, state3 = state
  )
...
rp.df <- rp.df %>%
  mutate(
    black_in_sworn = black_sworn / sworn,
...
rp.df <- rp.df %>%
  select(
    agency, zipcode, popserved, pop, city, state, state2, county, type,
    lat, lon,
...
rp.df %>%
  ggplot() +
  geom_density(aes(white_pop))

rp.df %>%
  ggplot() +
  geom_point(aes(x = white_pop, y = white_in_sworn), alpha = 0.2) +
  theme_minimal()
...
rp.df <-
  rp.df %>%
  mutate(
    white_rep = white_in_sworn - white_pop,
